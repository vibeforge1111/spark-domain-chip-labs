...
def test_creator_run_blocks_published_artifact_manifest_status(
    ...
    init_creator_run(
        run_dir,
        domain="Startup YC",
        goal="Test local artifact status boundary.",
    )

    manifest_path = run_dir / "created-artifact-manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["artifacts"][0]["status"] = "published"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
...
def test_creator_run_blocks_wrong_artifact_manifest_schema_version(
    tmp_path: Path,
) -> None:
    run_dir = tmp_path / "creator-run"
    init_creator_run(
        run_dir,
        domain="Startup YC",
        goal="Test local artifact manifest schema boundary.",
    )

    manifest_path = run_dir / "created-artifact-manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest["schema_version"] = "adaptive_creator_loop.created_artifact_manifest.v2"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    ...
    assert smoke.verdict == "blocked"
    assert any(
        check.name == "created_artifact_manifest_schema"
        and check.status == "fail"
        for check in smoke.checks
    )
