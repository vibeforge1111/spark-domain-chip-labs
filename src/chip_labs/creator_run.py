...
def init_creator_run(
    ...
    *,
    ...
    if not source_dir.exists():
        raise FileNotFoundError(f"Creator-run template directory not found: {source_dir}")
    ...
    for directory in ("domain-chip", "specialization-path", "benchmark", "autoloop", "reports", "swarm"):
    ...
    intent["domain"]["short_slug"] = domain.strip().lower().replace(" ", "-").replace("_", "-")
...
def validate_creator_run(run_dir: str | Path) -> SmokeResult:
    ...
    intent = _load_required_json(run_path / "creator-intent.json", "creator_intent", checks)
    adapter_map = _load_required_json(run_path / "adapter-map.json", "adapter_map", checks)

    if intent:
        _check_schema_prefix(intent, "adaptive_creator_loop.creator_intent.", "creator_intent_schema", checks)
        _check_non_empty(intent.get("run_id"), "creator_intent_run_id", "creator-intent.json has a run_id.", checks)
        _check_non_empty(
    ...
    if adapter_map:
        _check_schema_prefix(adapter_map, "adaptive_creator_loop.adapter_map.", "adapter_map_schema", checks)
        evidence_tier = str(_nested(adapter_map, "swarm_adapter", "evidence_tier") or "prototype")
...
def _load_required_json(path: Path, name: str, checks: list[SmokeCheck]) -> dict[str, Any] | None:
    if not path.exists():
...
def _check_non_empty(value: Any, name: str, pass_message: str, checks: list[SmokeCheck]) -> None:
...
def _check_adapter_sections(adapter_map: dict[str, Any], checks: list[SmokeCheck]) -> None:
    required_sections = (
        "domain_adapter",
        "benchmark_adapter",
        "tool_adapter",
    ...
    for section in required_sections:
        ...
        if isinstance(value, dict):
            checks.append(SmokeCheck(f"{section}_section", "pass", f"{section} exists."))
        else:
