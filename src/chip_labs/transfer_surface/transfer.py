...
def extract_patterns(chip_path: Path) -> list[TransferPattern]:
    ...
    # Cache file text once -- union of all glob patterns used by sub-extractors
    ...
    all_patterns.extend(
        _extract_loop_design_patterns(chip_path, chip_name, evidence_strength, project, all_text)
    )
    all_patterns.extend(
        _extract_evidence_strategy_patterns(chip_path, chip_name, evidence_strength, all_text)
    )
    all_patterns.extend(
        _extract_promotion_gate_patterns(chip_path, chip_name, evidence_strength, project, all_text)
    )
    all_patterns.extend(
        _extract_contradiction_detection_patterns(chip_path, chip_name, evidence_strength, all_text)
    )
    all_patterns.extend(
