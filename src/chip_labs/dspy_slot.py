...
def _python_string_literal(value: str, *, label: str) -> str:
    """Return an exact, injection-safe Python literal for a config string."""
...
def _require_identifier(value: str, *, label: str) -> None:
    """Reject names that cannot be used consistently in generated Python."""
    if (
...
def generate_slot_script(config: DSpySlotConfig) -> str:
    ...
    """
    _python_string_literal(config.slot_name, label="slot name")
    _python_string_literal(config.model, label="model")
    _python_string_literal(config.task_description, label="task description")
    _python_string_literal(config.training_data_path, label="training data path")
    _require_identifier(config.metric_name, label="metric name")
    if (
        isinstance(config.max_bootstrapped_demos, bool)
        or not isinstance(config.max_bootstrapped_demos, int)
        or config.max_bootstrapped_demos < 0
    ):
        raise ValueError("max bootstrapped demos must be a non-negative integer")
    for field_name, field_description in config.input_fields.items():
        _require_identifier(field_name, label="input field name")
        _python_string_literal(field_description, label=f"input field {field_name} description")
    for field_name, field_description in config.output_fields.items():
        _require_identifier(field_name, label="output field name")
        _python_string_literal(field_description, label=f"output field {field_name} description")
    ...
    for fname, fdesc in config.input_fields.items():
        input_field_lines.append(
            f"    {fname}: str = dspy.InputField("
            f"desc={_python_string_literal(fdesc, label=f'input field {fname} description')})"
        )

    output_field_lines = []
    for fname, fdesc in config.output_fields.items():
        output_field_lines.append(
            f"    {fname}: str = dspy.OutputField("
            f"desc={_python_string_literal(fdesc, label=f'output field {fname} description')})"
        )
    ...
    for k in input_keys:
        run_arg_lines.append(
            ...
            f"{_python_string_literal(f'--{k}', label='input option')}, "
            "required=True, "
            f"help={_python_string_literal(config.input_fields[k], label=f'input field {k} description')})"
        )
    ...
    with_inputs_arg = ", ".join(
        _python_string_literal(k, label="input field name") for k in input_keys
    )
    ...
    module_doc = (
        f"DSPy slot script: {config.slot_name}\n\n{config.task_description}\n\n"
        "Generated by spark-domain-chip-labs dspy_slot framework."
    )
    parts.append("#!/usr/bin/env python3")
    parts.append(_python_string_literal(module_doc, label="module docstring"))
    parts.append("")
    ...
    parts.append("")
    parts.append(
        f"SLOT_NAME = {_python_string_literal(safe_name, label='sanitized slot name')}"
    )
    parts.append(
        "MODEL = os.environ.get(\"DSPY_MODEL\", "
        f"{_python_string_literal(config.model, label='model')})"
    )
    parts.append(
        "TRAINING_DATA_PATH = "
        f"{_python_string_literal(config.training_data_path, label='training data path')}"
    )
    parts.append(f"MAX_BOOTSTRAPPED_DEMOS = {config.max_bootstrapped_demos}")
    parts.append(
        "TASK_DESCRIPTION = "
        f"{_python_string_literal(config.task_description, label='task description')}"
    )
    parts.append("")
    parts.append("")
    parts.append("# ---------------------------------------------------------------------------")
    ...
    parts.append(
        ...
        + _python_string_literal(
            config.task_description or config.slot_name,
            label="signature docstring",
        )
    )
    parts.append("")
    ...
    parts.append(
        "    "
        + _python_string_literal(
            f"DSPy module for {config.slot_name}.",
            label="module class docstring",
        )
    )
    parts.append("")
    ...
    parts.append(f"        payload = {{k: getattr(result, k, None) for k in {output_keys!r}}}")
    parts.append('        if getattr(sys.stdout, "isatty", lambda: False)():')
    parts.append("            print(json.dumps(payload, indent=2))")
    parts.append("        else:")
    parts.append('            print(json.dumps(payload, separators=(",", ":")))')
    parts.append("")
    parts.append("")
