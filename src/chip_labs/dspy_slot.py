...
def _escape_for_python_string(s: str) -> str:
    """Escape a string for safe interpolation into Python source code.

    Prevents code injection via malicious config values in generated scripts.
    """
    if not s:
        return ""
    s = s.replace("\\", "\\\\")
...
def generate_slot_script(config: DSpySlotConfig) -> str:
    ...
    for fname, fdesc in config.input_fields.items():
        input_field_lines.append(
            f'    {fname}: str = dspy.InputField(desc="{_escape_for_python_string(fdesc)}")'
        )

    output_field_lines = []
    for fname, fdesc in config.output_fields.items():
        output_field_lines.append(
            f'    {fname}: str = dspy.OutputField(desc="{_escape_for_python_string(fdesc)}")'
        )
    ...
    for k in input_keys:
        run_arg_lines.append(
            f'    run_parser.add_argument("--{k}", required=True, help="{_escape_for_python_string(config.input_fields[k])}")'
        )
    ...
    task_desc_escaped = _escape_for_python_string(config.task_description)
    ...
    parts.append(f'#!/usr/bin/env python3\n"""DSPy slot script: {_escape_for_python_string(config.slot_name)}\n\n{_escape_for_python_string(config.task_description)}\n\nGenerated by spark-domain-chip-labs dspy_slot framework.\n"""')
    parts.append("")
    ...
    parts.append(f'MODEL = os.environ.get("DSPY_MODEL", "{_escape_for_python_string(config.model)}")')
    parts.append(f'TRAINING_DATA_PATH = "{_escape_for_python_string(config.training_data_path)}"')
    ...
    parts.append(f'    """{_escape_for_python_string(config.task_description or config.slot_name)}"""')
    parts.append("")
