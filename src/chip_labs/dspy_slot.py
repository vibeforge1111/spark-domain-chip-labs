...
def generate_slot_script(config: DSpySlotConfig) -> str:
    ...
    parts.append("        # Pretty-print when a human is at the terminal; emit compact")
    parts.append("        # single-line JSON when stdout is a pipe / file redirect so")
    parts.append("        # `| jq`, `| grep`, ndjson pipelines, and line-oriented shell")
    parts.append("        # loops can consume the result without indent=2 whitespace.")
