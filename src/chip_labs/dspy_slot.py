...
def generate_slot_script(config: DSpySlotConfig) -> str:
    ...
    model_escaped = config.model.replace("\", "\\").replace('"', '\\"')
    training_path_escaped = config.training_data_path.replace("\", "\\").replace('"', '\\"')
    parts.append(f'MODEL = os.environ.get("DSPY_MODEL", "{model_escaped}")')
    parts.append(f'TRAINING_DATA_PATH = "{training_path_escaped}"')
