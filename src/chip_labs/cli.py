...
def cmd_portfolio_v3(args: argparse.Namespace) -> None:
    ...
    if ranking:
        ...
        print("Ranking: no domain chips discovered")
...
def cmd_creator_run_doctor(args: argparse.Namespace) -> None:
...
def cmd_creator_run_template_check(args: argparse.Namespace) -> None:
    ...
    result = validate_creator_templates(args.template_dir)
    _write_output(args.output, result)
    if args.fail_on_blocked and result["verdict"] == "blocked":
        raise SystemExit(1)
...
def cmd_generated_multi_seed_run(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        raise SystemExit(f"--briefs file not found: {briefs_path}") from exc
    except json.JSONDecodeError as exc:
        raise SystemExit(f"--briefs is not valid JSON: {briefs_path}") from exc
...
def cmd_startup_yc_heldout_check(args: argparse.Namespace) -> None:
    """Check Startup YC held-out founder-advice evidence."""
    from .startup_yc_promotion import check_startup_yc_heldout_validation
...
def main() -> None:
    ...
    # creator-run-doctor
    p_creator_doctor = sub.add_parser(
        "creator-run-doctor",
        help="Diagnose a creator-run workspace and emit concrete repair steps.",
    )
    p_creator_doctor.add_argument("run_dir", type=str, help="Creator-run directory.")
    ...
    p_creator_template_check = sub.add_parser(
        "creator-run-template-check",
        help="Validate the creator-run template set before shipping creator contracts.",
    )
    p_creator_template_check.add_argument(
        "--template-dir", type=str, default=None, help="Template directory to check."
    )
    p_creator_template_check.add_argument(
        "--output", type=str, default=None, help="Output JSON file path."
    )
    ...
    p_startup_yc_multi_seed_check = sub.add_parser(
        "startup-yc-multi-seed-check",
        help="Check Startup YC multi-seed validation evidence.",
    )
    p_startup_yc_multi_seed_check.add_argument(
        "--validation-plan",
        type=str,
        required=True,
        help="Startup YC validation_plan.json path.",
    ...
    p_startup_yc_multi_seed_check.add_argument(
        "--fail-on-blocked",
        action="store_true",
        help="Exit with status 1 when multi-seed evidence is blocked.",
    )
    p_startup_yc_multi_seed_check.set_defaults(func=cmd_startup_yc_multi_seed_check)

    # startup-yc-heldout-check
    p_startup_yc_heldout_check = sub.add_parser(
        "startup-yc-heldout-check",
        help="Check Startup YC held-out founder-advice evidence.",
    )
    p_startup_yc_heldout_check.add_argument(
        "--validation-plan",
        type=str,
        required=True,
        help="Startup YC validation_plan.json path.",
